...
class Store(AbstractUser): 
    ...
    store_name = models.CharField(max_length=50) # 상호명
    store_img = models.ImageField(upload_to='img') # 스토어 이미지
    num = models.CharField(max_length=20) # 사업자번호 [유효성 검사 있으면 좋을듯]
    address = models.CharField(max_length=50) # 회사 주소 [주소검색 api를 통해서 입력받으면 좋을텐데 어려우면 패스]
    zipcode = models.IntegerField() # 회사 우편번호
    email = models.EmailField(max_length=50) # 이메일
    telnum = models.CharField(max_length=50) # 연락처 폰번호 양식 유효성검사 있으면 좋을듯
